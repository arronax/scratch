#!/bin/bash
for file in [in-memory,io-bound]*log; do
  grep ".*,.*,.*," $file > $file.processed.csv
  awk '/SQL statistics/,0' $file > $file.stats
done
for file in [in-memory,io-bound]*processed*; do
  label=$(echo "${file}" | awk -F'.' '{print $1}')
  arch=$(echo "${file}" | awk -F'.' '{print $2}')
  mv "${file}" "${label}.${arch}.csv"
done
for file in [in-memory,io-bound]*.log.stats; do
  label=$(echo "${file}" | awk -F'.' '{print $1}')
  arch=$(echo "${file}" | awk -F'.' '{print $2}')
  mv "${file}" "${label}.${arch}.stats"
done
